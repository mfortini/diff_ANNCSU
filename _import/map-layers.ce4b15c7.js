...
export function createLayerManager({
  ...
  // Stack z-order: date più vecchie sotto, più nuove sopra (indipendente da quando
  // l'utente le attiva). I confini regione/comune restano in cima.
  function reorderDateLayers(targetMap, prefix) {
    const ordered = [];
    for (const date of [...DATES].sort()) {
      forEachDatePointLayerId(prefix, date, (layerId) => {
        if (targetMap.getLayer(layerId)) ordered.push(layerId);
      });
    }
    ...
    for (const bid of [
      ...
      "comune-boundary-fill",
      "comune-boundary-line"
    ]) {
      if (targetMap.getLayer(bid)) targetMap.moveLayer(bid);
    }
  }
  ...
  async function ensureDateLayersForMap(targetMap, prefix, date) {
    ...
    loaded.add(date);
    reorderDateLayers(targetMap, prefix);
  }
